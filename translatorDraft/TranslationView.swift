...
struct TranslationView: View {
    @State private var sourceLanguageIndex = 0
        ...
        let languageCodes = ["tr", "en", "de", "es", "it", "ru", "ar"]
        ...
        func translationText() {
            ...
            translate(text: inputText, from: sourceLanguageCode, to: targetLanguageCode) { translation in
                DispatchQueue.main.async {
                    self.outputText = translation
                }
            }
        }
        
        func translate(text: String, from: String, to: String, completion: @escaping (String) -> Void) {
            let urlStr = "https://api.mymemory.translated.net/get?q=\(text.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")&langpair=\(from)|\(to)"
        ...
        func parseTranslation(data: Data) -> String? {
            do {
